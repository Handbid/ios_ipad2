...
public extension DecodingError {
	var description: String? {
		...
		case let .typeMismatch(_, value):
			"typeMismatch error: \(value.debugDescription)  \(localizedDescription)"
		case let .valueNotFound(_, value):
			"valueNotFound error: \(value.debugDescription)  \(localizedDescription)"
		case let .keyNotFound(_, value):
			"keyNotFound error: \(value.debugDescription)  \(localizedDescription)"
		case let .dataCorrupted(key):
			"dataCorrupted error at: \(key)  \(localizedDescription)"
		default:
			"decoding error: \(localizedDescription)"
		}
	}
}
