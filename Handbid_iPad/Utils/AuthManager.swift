...
class AuthManager: ObservableObject {
	...
	func checkUserStatus() async -> Bool {
		do {
			guard let tokenData = try? Keychain(service: AppInfoProvider.bundleIdentifier).getData("AuthDataUser"),
	...
	func isLoggedInAsync() async throws -> Bool {
		try await withCheckedThrowingContinuation { continuation in
			Task {
				do {
					guard let tokenData = try? Keychain(service: AppInfoProvider.bundleIdentifier).getData("AuthDataUser") else {
